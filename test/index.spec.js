...
describe("index.js", () => {
  ...
  it("Should store stuff in the Database when calling POST '/api/shouts' route", async (done) => {
    await expect(
      request(app).post("/api/shouts").send({ username: "TESTING SCRIPT", message: "TESTING DATA" })
